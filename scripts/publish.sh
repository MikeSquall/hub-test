...
set_git_remote() {
    git remote set-url origin https://${GH_ACCESS_TOKEN}@github.com/MikeSquall/hub-test
}
...
if [[ "$TRAVIS_PULL_REQUEST_BRANCH" =~ ^feature\/.*$ ]]; then
    ...
    echo "publish command --> lerna publish --canary --dist-tag snapshot --preid $PACKAGE_VERSION --yes"
    echo_result "$?"
    echo "comment command --> lerna run publish:comment -- alpha $PACKAGE_VERSION"
    ...
    echo "publish command --> lerna publish prerelease --canary --preid next --dist-tag next --yes -m '[skip ci]'"
    echo_result "$?"
    ...
    set_git_remote
    git checkout master
    echo "publish command --> lerna publish patch --yes -m '[skip ci]'"
    echo_result "$?"
